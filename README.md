# HTX Number Checker API — Bulk Verification | NumberChecker

The **HTX Number Checker API** accepts a text file of phone numbers, creates an asynchronous bulk task, and exports a result file. This repository contains a runnable integration in eight languages.

- **Product page:** https://numberchecker.ai/products/htx-checker
- **API documentation:** https://docs.numberchecker.ai/htx-bulk-number-checker
- **API base URL:** `https://api.numberchecker.ai`
- **Authentication:** `X-API-Key`
- **Get an API key:** https://platform.numberchecker.ai

> [!NOTE]
> This repository covers the standalone product code `htx` only. Other product codes are outside its scope.
...
| Minimum batch | 2,000 identifiers |
...
1. Submit the input file to `POST /v1/tasks` with `task_type=htx`.
...
Only `htx`. The task type is intentionally fixed to that product code.
...
- **Product:** https://numberchecker.ai/products/htx-checker
- **Documentation:** https://docs.numberchecker.ai/htx-bulk-number-checker
